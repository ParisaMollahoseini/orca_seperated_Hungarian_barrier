...
namespace RVO {
	...
	void Agent::computeNeighbors()
	{
		...
		for (int i = 0; i < barriers_.size(); i++)
		{
			...
			if ((abs(position_ - barriers_[i].position_) < neighborDist_))
			{

				float x = neighborDist_ * neighborDist_;
				insertAgentNeighbor(&barriers_[i], x);
	...
	void Agent::computeNewVelocity()
	{
		...
		const size_t planeFail = linearProgram3(orcaPlanes_, maxSpeed_, prefVelocity_, false, newVelocity_);//change

		if (planeFail < orcaPlanes_.size()) {
			linearProgram4(orcaPlanes_, planeFail, maxSpeed_, newVelocity_);//change
		}
	}
	...
	void Agent::update()
	{
		...
		for (int i = 0; i < barriers_.size(); i++)
		{
			if (barriers_[i].velocity_ != Vector3(0, 0, 0))
			{
				barriers_[i].position_ = barriers_[i].position_ + (barriers_[i].velocity_ * timeStep_);
			}
		}
		...
		if (absSq(position_ - agent_goal) <= (4.0f * radius_ * radius_))
		{
			...
			//std::cout << "position:  " << position_ << "-------" << absSq(position_ - agent_goal)<<std::endl;
		}
	}
