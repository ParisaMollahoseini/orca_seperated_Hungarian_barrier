...
def create_surface(ax, redus, x, y, z, c_index, agent_num, is_obstacle):
    ...
    # Plot the surface
    color_index = c_index % min(len(color_list)-1, agent_num)
    if is_obstacle:
        shape = ax.plot_surface(x, y, z, color=color_list[7])
    else:
        shape = ax.plot_surface(x, y, z, color=color_list[2])
    return shape 
...
def main():
    ...
    parser.add_argument("-s", "--interval_speed", help="interval speed", default=10)
    ...
    REDUCE = float(config['agent_reduce'])
    ...
    def update_points(index, single_log, main_list, points):
        ...
        for point,coordinate in zip(points, states):
            ...
            if coordinate[3] == 1:
                create_surface(ax, REDUCE, new_x, new_y, new_z, color_ind, AGENT_NUMBER, False)
            else:
                create_surface(ax, REDUCE, new_x, new_y, new_z, color_ind, AGENT_NUMBER, True)
